...
3. 설정 구성:
   ...
   web_auth_enabled: false          # Web UI 인증 활성화
   web_auth_username: "admin"       # 인증 사용자명
   web_auth_password: "password"    # 인증 비밀번호
   ```
...
| `WEB_AUTH_ENABLED` | Web UI 인증 활성화 | false |
| `WEB_AUTH_USERNAME` | Basic Auth 사용자명 | - |
| `WEB_AUTH_PASSWORD` | Basic Auth 비밀번호 | - |
...
인증이 활성화된 경우, 스타일링된 로그인 페이지가 표시됩니다.

![로그인](images/login.png)

### 대시보드

실시간 상태 모니터링 화면입니다. Upstream 연결 상태, 클라이언트 수, 가동 시간을 확인할 수 있으며, 실시간 로그를 통해 시스템 이벤트를 모니터링합니다.

![대시보드](images/dashboard.png)

### 패킷 인스펙터

실시간으로 패킷을 모니터링하고 분석할 수 있습니다.
...
![패킷 인스펙터](images/packet-inspector.png)

### 클라이언트 관리

연결된 TCP 클라이언트 및 Web 클라이언트 목록을 확인하고, 필요시 연결을 해제할 수 있습니다.

![클라이언트 관리](images/clients.png)

### 패킷 인젝션

테스트 패킷을 Upstream(서버) 또는 Downstream(클라이언트)으로 전송할 수 있습니다. HEX 또는 ASCII 형식으로 데이터를 입력합니다.

![패킷 인젝션](images/inject.png)
